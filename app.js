...
async function enterPortal() {
  // The company comes from the database, keyed on the signed-in user. The
  // page never chooses it, so editing anything here changes nothing.
  const { data, error } = await sb.from('portal_users')
    .select('company_name, status').eq('status', 'Active').limit(1);

  if (error || !data || !data.length) {
    document.getElementById('portalLoginError').textContent =
      'This account is not yet linked to a company. Please contact us and we will set it up.';
    await sb.auth.signOut();
    session = null;
    return;
  }
  myCompany = data[0].company_name;
...
async function handlePortalLogout() {
  ...
  session = null; myCompany = ''; queuedLineItems = [];
